...
class Character {
    ...
    func attack (target: Character) {
        ...
        if weapon.name == "Dague à double tranchant des exilés" {
            self.hp -= 15; print((weapon.name), "a fait perdre 15 point de vie ♥ à", (self.name))
        }
        
        print( self.name + " attaque " + target.name )
        print ( target.name , " a perdu " , self.weapon.actionspoints , "point de vie ♥")
        print(" il reste " , target.hp , " point de vie ♥ à ", target.name  )
    ...
    func heal(target:Character){
        ...
        print(target.name, " a gagné", self.weapon.actionspoints, "point de vie ♥")
        print("il reste",target.hp,"de point de vie ♥ à", target.name)
    ...
    func weaponBonusMalus() {
        ...
        if weapon.name == "Lame maudite du monde des morts" {
            self.hp = 1; print((self.name), " semble soudain entre la vie et la mort mais la lame maudite semble incroyablement destructrice", "il reste", (self.hp), "de vie ♥ à", (self.name))
    }
